...
This repo is a Chezmoi-managed home directory source for macOS and Linux. The canonical checkout lives at `~/.local/share/chezmoi`. Chezmoi source names use attributes such as `dot_`, `private_`, `executable_`, and `encrypted_`: `dot_zshrc` applies to `~/.zshrc`, `dot_config/nvim/` applies to `~/.config/nvim/`, and `dot_local/bin/executable_tmux-sessionizer` applies as an executable helper script. Source-root files such as `README.md`, `AGENTS.md`, `Brewfile`, and `LinuxPackages` support the repo and are not applied to `$HOME`.
